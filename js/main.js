/* ==========================================================================
   MAIN CORE ARCHITECTURE CONTEXT MAPPING ORCHESTRATOR
   ========================================================================== */
...
if (audioBtn) {
  audioBtn.addEventListener('click', () => {
    ...
    if (!isMuted) { bgMusic.play().catch(err => { console.log("Audio streaming failure check:", err); }); } else { bgMusic.pause(); }
  });
}

/* ── VIDEO REPLAY SYSTEM PIPELINE MECHANICS ── */
const replayBtn = document.getElementById('video-replay-toggle');
const introContainer = document.getElementById('intro');
const introVideo = document.getElementById('intro-video');
const skipIntroBtn = document.getElementById('skip-intro-btn');
const introCanvasElement = document.getElementById('intro-canvas');

if (replayBtn && introContainer && introVideo) {
  replayBtn.addEventListener('click', () => {
    if (typeof bgMusic !== 'undefined' && !bgMusic.paused) {
      ...
      if (audioToggleBtn) {
        ...
        audioToggleBtn.style.color = 'var(--muted)';
        audioToggleBtn.style.borderColor = 'rgba(108,47,255,0.2)';
        isMuted = true;
      }
    }
    introContainer.style.display = 'flex';
    introContainer.style.opacity = '1';
    if (introCanvasElement) introCanvasElement.style.display = 'none';
    introVideo.style.display = 'block';
    introVideo.classList.add('visible');
    if (skipIntroBtn) skipIntroBtn.style.display = 'block';
    introVideo.muted = false;
    introVideo.currentTime = 0;
    introVideo.play().catch(err => { console.log("Replay execution alignment halt:", err); });
  });
}

/* ── PILOT GAME CONTROL MODULE INITIALIZATION ── */
...
if (gameBtn) {
  gameBtn.addEventListener('click', () => {
    ...
    gameBtn.style.color = window.isGameMode ? 'var(--cyan)' : 'var(--muted)';
    gameBtn.style.borderColor = window.isGameMode ? 'var(--cyan)' : 'rgba(108,47,255,0.2)';
    gameBtn.style.boxShadow = window.isGameMode ? '0 0 10px var(--cyan)' : 'none';
  });
}