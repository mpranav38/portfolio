/* ==========================================================================
   PERFECT AMBIENT BACKGROUND SYSTEM: ENDLESS PURSUIT CORE (NO HIT LOCKS)
   ========================================================================== */
...
// --- PERSISTENT COSMIC BATTLE ENGINE ---
const SHIPS = [];
const LASERS = [];

if (!window.isMob) {
    // Clean initialized properties without messy collision variables
...
function updateBattleSystem() {
    ...
    if (cyanHunter && redTarget) {
        ...
        // Firing Mechanis: Steady and smooth background decoration rate
        ...
        if (cyanHunter.cooldown === 0 && inBounds && distance < 600) {
            // Aim with slight prediction tracking
            ...
            cyanHunter.cooldown = 32; 
        }
    }

    // 3. Process Laser Motion (No collision processing loops = ZERO STICKING)
...
window.addEventListener('click', e => {
    ...
    gravityTimer = 90; 
    if (window.playCosmicSound) window.playCosmicSound('implode');
}, { passive: true });
...
/* ── MASTER SINGLE-LOOP RENDERING ENGINE ── */
let T = 0;
function renderEngine() {
    ...
    // ── UFO DESIGN RENDERING ENGINE ──
    if (!window.isMob) {
        SHIPS.forEach(ufo => {
            sx.save();
            sx.translate(ufo.x, ufo.y);
            
            const colorGlow = ufo.team === 'red' ? '#ff3333' : varColor('--cyan');
            sx.strokeStyle = colorGlow;
            sx.fillStyle = 'rgba(10, 6, 32, 0.9)'; 
            sx.lineWidth = ufo.team === 'red' ? 2.5 : 2;

            sx.beginPath();
            sx.ellipse(0, 0, ufo.r * 1.5, ufo.r * 0.5, 0, 0, Math.PI * 2);
            sx.fill(); sx.stroke();

            sx.beginPath();
            sx.arc(0, -ufo.r * 0.15, ufo.r * 0.65, Math.PI, 0);
            sx.fillStyle = ufo.team === 'red' ? 'rgba(255, 51, 51, 0.15)' : 'rgba(0, 229, 255, 0.1)';
            sx.fill(); sx.stroke();

            sx.save();
            sx.rotate(ufo.spinAngle);
            const bulbCount = ufo.team === 'red' ? 7 : 6;
            for (let i = 0; i < bulbCount; i++) {
                const nodeAngle = (i / bulbCount) * Math.PI * 2;
                const nodeX = Math.cos(nodeAngle) * (ufo.r * 1.1);
                const nodeY = Math.sin(nodeAngle) * (ufo.r * 0.35);
                
                sx.beginPath();
                sx.arc(nodeX, nodeY, 2, 0, Math.PI * 2);
                sx.fillStyle = (Math.floor(T * 10) + i) % 2 === 0 ? colorGlow : '#ffffff';
                sx.fill();
            }
            sx.restore();
            sx.restore();
        ...
        // Fired Plasma Spheres (Matches portfolio Cyan theme perfectly)
        LASERS.forEach(l => {
            sx.save();
            sx.beginPath();
            sx.arc(l.x, l.y, 4.5, 0, Math.PI * 2); 
            const lColor = varColor('--cyan');
            sx.fillStyle = '#ffffff';
            sx.strokeStyle = lColor;
            sx.lineWidth = 2;
            sx.shadowBlur = 12; sx.shadowColor = lColor;
            sx.fill(); sx.stroke();
            sx.restore();
        });
    }
    ...
    function varColor(variable) {
        return document.body.classList.contains('supernova') ? '#4c1bc7' : (variable === '--cyan' ? '#00e5ff' : '#ff3cac');
    }
    ...
    PARTS.forEach(p => {
        ...
        px.beginPath(); px.arc(p.x + mox, p.y + moy, p.r, 0, Math.PI * 2);
        px.fillStyle = `hsla(${p.hue},70%,75%,${p.a})`; px.fill();
    });

    if (gravityTimer > 0) {
        gravityTimer--;
        if (gravityTimer === 0) gravityWell = null;
    }
    ...
    if (!window.isMob) {
        TRAIL.forEach((p, i) => {
            ...
            tx2.beginPath(); tx2.arc(p.x, p.y, p.r * frac * p.life, 0, Math.PI * 2);
            tx2.fillStyle = `hsla(${220 + frac * 60},80%,75%,${p.life * frac * .4})`; tx2.fill();
        });
    }
...
if (!window.isMob) {
    ...
    gx.strokeStyle = 'rgba(108,47,255,.04)'; gx.lineWidth = 1;
    const size = 60;
